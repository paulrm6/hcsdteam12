...
public class CalendarFrame extends javax.swing.JFrame implements ActionListener, MouseListener {
	...
	//Launches calendar frame and retrive information about current date from DetailsManager. In the process, it launches the GUI components into the frame. 
	...
	public void reviseCalendar(int month, int year) {
		int firstDayOfMonth = LeapYearFunc.DayOneOfMonth(year, month); //Identifies the day where the 1st of a month sits on
		...
		for(int i = 0; i < LeapYearFunc.RowsInMonth(year,month); i++) {
		...
		if(extraDaysPrior > 0) {
			...
			indexDayOne = LeapYearFunc.RowsInMonth(year, month+before) - extraDaysPrior;
		}
